  ...
  return (
    <section className="grid gap-5" aria-labelledby="appearance-settings-title">
      ...
      <div className="grid grid-cols-[1fr_auto] items-center gap-3 border-y border-border py-3">
        ...
        <Select value={document.appearance.mode} disabled={busy} onValueChange={(mode) => void execute({ type: "appearance.setMode", mode: mode as "system" | "light" | "dark" })}>
            ...
            return (
              <div key={theme.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 py-2">
                ...
                <div className="flex items-center gap-1">
                  <Button type="button" size="xs" variant={active ? "secondary" : "ghost"} aria-pressed={active} disabled={busy || active} onClick={() => void activate(theme.id)}>{active ? "Active" : "Activate"}</Button>
                  <Button type="button" size="xs" variant="ghost" onClick={() => setEditing(theme)}>{custom ? "Edit" : "Customize"}</Button>
                  <Button type="button" size="xs" variant="ghost" disabled={busy} onClick={() => void exportTheme(theme.id)}>Export</Button>
                  {custom && <Button type="button" size="xs" variant="ghost" disabled={busy} onClick={() => setDeleteTheme(theme)}>Delete</Button>}
                </div>
              </div>
            );
          })}
