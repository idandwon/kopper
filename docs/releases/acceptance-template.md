...
| ONBOARD-03 | Select **Repair access**, enable the current Kopper entry under **System Settings > Privacy & Security > Accessibility**, and return to Kopper. Confirm focus/polling detects the grant without a manual check button. | Not run | Not run — protected installed app and physical clean-account onboarding required |
