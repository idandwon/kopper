...
| Accepted `expected_dmg_sha256` input | Not run — exact `<64-character lowercase DMG SHA-256>` recorded before promotion |
...
- Promotion requires the exact `tag`, `expected_commit`, and `expected_dmg_sha256` values recorded in this acceptance record. Do not approve or dispatch promotion with values copied from a later checkout or re-download.
...
| UNSIGNED-02 | The root-level real `Kopper.app` reports the exact version, bundle identifier `com.kopper.app`, minimum macOS `14.0`, and exactly the `arm64` and `x86_64` runtime architecture sets. | Not run | Not run — mount the draft DMG read-only and record bounded metadata and both normalized `lipo` outputs. |
