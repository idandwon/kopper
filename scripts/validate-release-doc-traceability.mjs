...
function parseTables(markdown, relativePath) {
  ...
  for (let index = 0; index < lines.length - 1; index += 1) {
    if (!lines[index].trim().startsWith('|') || !/^\s*\|(?:\s*:?-+:?\s*\|)+\s*$/.test(lines[index + 1])) {
      continue
    }
...
function collectIdRows(tables) {
  return tables.flatMap((table) =>
    table.rows
      ...
      .map(({ cells, line }) => ({ id: cells[0], meaning: cells[1], cells, line, header: table.header })),
  )
}
...
const canonicalRows = collectIdRows(parseTables(procedure, 'tests/manual/macos-capture.md'))
const canonicalIds = new Set(canonicalRows.map(({ id }) => id))

if (canonicalRows.length === 0 || canonicalIds.size !== canonicalRows.length) {
  errors.push('Canonical procedure must contain a non-empty, duplicate-free ID sequence.')
}
...
for (const recordPath of recordPaths) {
  ...
  for (const row of recordRows) counts.set(row.id, (counts.get(row.id) ?? 0) + 1)
  for (const { id } of canonicalRows) {
    if (counts.get(id) !== 1) errors.push(`${relativePath}: ${id} must appear exactly once.`)
  }

  for (const row of recordRows) {
    const statusIndex = row.header.findIndex((cell) => cell === 'Status')
    const evidenceIndex = row.header.findIndex((cell) => /Evidence|blocker/i.test(cell))
    if (statusIndex < 0 || evidenceIndex < 0) {
      errors.push(`${relativePath}:${row.line}: ${row.id} needs its own Status and Evidence/blocker cells.`)
      continue
    }
    if (!['Pass', 'Fail', 'Not run'].includes(row.cells[statusIndex])) {
      errors.push(`${relativePath}:${row.line}: ${row.id} has invalid status ${JSON.stringify(row.cells[statusIndex])}.`)
    }
    if (!row.cells[evidenceIndex]) {
      errors.push(`${relativePath}:${row.line}: ${row.id} has an empty Evidence/blocker cell.`)
    }
  }
}

if (errors.length > 0) {
  console.error(`Release documentation traceability validation failed (${errors.length} error(s)):\n`)
  console.error(errors.join('\n\n'))
  process.exitCode = 1
} else {
  console.log(`Release documentation traceability valid: ${canonicalRows.length} canonical rows match in 2 acceptance records.`)
}
