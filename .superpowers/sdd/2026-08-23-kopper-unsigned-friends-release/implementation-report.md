...
- Apple credentials absent from active path: the required credential/signing scan returned only `README.md:67` (explicitly says no repository or environment secrets) and `scripts/workflows.test.ts:76` (a negative assertion). It found no active credential, signing, notarization, or stapling use in `.github`, `package.json`, `install.sh`, or release scripts.
- Signing/notarization explicitly disabled: `package.json` defines `package:beta` as `pnpm build && electron-builder --mac dmg --universal -c.mac.identity=null -c.mac.notarize=false`.
